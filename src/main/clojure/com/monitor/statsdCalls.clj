...
; use these if it makes more sense to redeclare the udp server that simply hold it static....
...
(defn tagString [& args]
  (if (= (length args) 0)       ; in progress!!! not sure on if _or_ fold syntax
    ""
    (reduce-kv (a b -> a + "|" + b) "|#" args))
  )
;
